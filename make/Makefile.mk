...
HELMFILE_GLOBAL_FLAGS += $(if $(findstring true,$(HELMFILE_USE_SELECTORS)),$(shell get-selectors-and-clean.sh $(OUTPUT_DIR)),)
...
# NOTE be very careful about these 2 labels as getting them wrong can remove stuff in you cluster!
	if [ -d $(OUTPUT_DIR)/customresourcedefinitions ]; then \
	  kubectl apply $(KUBECTL_APPLY_FLAGS) --server-side --prune -l=gitops.jenkins-x.io/repository=$(REPO_NAME),gitops.jenkins-x.io/pipeline=customresourcedefinitions -R -f $(OUTPUT_DIR)/customresourcedefinitions; \
	fi
	kubectl apply $(KUBECTL_APPLY_FLAGS) --server-side --prune -l=gitops.jenkins-x.io/repository=$(REPO_NAME),gitops.jenkins-x.io/pipeline=cluster						-R -f $(OUTPUT_DIR)/cluster
	kubectl apply $(KUBECTL_APPLY_FLAGS) --server-side --prune -l=gitops.jenkins-x.io/repository=$(REPO_NAME),gitops.jenkins-x.io/pipeline=namespaces				-R -f $(OUTPUT_DIR)/namespaces
...
# lets ignore commit errors in case there's no changes and to stop pipelines failing
	-git commit -m "chore: regenerated" -m "/pipeline cancel [skip ci]"
